...
public class DynamoDbStreamsSourceEnumerator
        implements SplitEnumerator<DynamoDbStreamsShardSplit, DynamoDbStreamsSourceEnumeratorState> {

    private static final Logger LOG = LoggerFactory.getLogger(DynamoDbStreamsSourceEnumerator.class);
    ...
    private List<DynamoDbStreamsShardSplit> mapToSplits(
        ...
        switch (initialPosition) {
            case LATEST:
                ...
                break;
            case AT_TIMESTAMP:
                throw new UnsupportedOperationException("DDB Stremas does not support shard iterator at timestamp");
            case TRIM_HORIZON:
            default:
    ...
    private void assignSplitToSubtask(
            DynamoDbStreamsShardSplit split, Map<Integer, List<DynamoDbStreamsShardSplit>> newSplitAssignments) {
    ...
    private void updateSplitAssignment(Map<Integer, List<DynamoDbStreamsShardSplit>> newSplitsAssignment) {
    ...
    private static class ShardAssignerContext implements DynamoDbStreamsShardAssigner.Context {
        ...
        private ShardAssignerContext withPendingSplitAssignments(
            ...
            Map<Integer, List<DynamoDbStreamsShardSplit>> copyPendingSplitAssignments = new HashMap<>();
        ...
        public Map<Integer, Set<DynamoDbStreamsShardSplit>> getCurrentSplitAssignment() {
            Map<Integer, Set<DynamoDbStreamsShardSplit>> copyCurrentSplitAssignment = new HashMap<>();
            for (Entry<Integer, Set<DynamoDbStreamsShardSplit>> entry : splitAssignment.entrySet()) {
