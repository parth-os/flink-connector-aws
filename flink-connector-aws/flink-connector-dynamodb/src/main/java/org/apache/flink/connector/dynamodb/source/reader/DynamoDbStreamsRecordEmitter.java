...
public class DynamoDbStreamsRecordEmitter<T>
    ...
    private final DynamoDbStreamsDeserializationSchema<T> deserializationSchema;
    private final SourceOutputWrapper<T> sourceOutputWrapper = new SourceOutputWrapper<>();

    public DynamoDbStreamsRecordEmitter(DynamoDbStreamsDeserializationSchema<T> deserializationSchema) {
        this.deserializationSchema = deserializationSchema;
    }

    @Override
    public void emitRecord(
            Record element, SourceOutput<T> output, DynamoDbStreamsShardSplitState splitState)
            throws Exception {
        sourceOutputWrapper.setSourceOutput(output);
        sourceOutputWrapper.setTimestamp(element.dynamodb().approximateCreationDateTime().toEpochMilli());
