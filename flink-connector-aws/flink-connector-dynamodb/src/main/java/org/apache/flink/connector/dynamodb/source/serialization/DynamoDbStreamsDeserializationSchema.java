 ...
 */
@PublicEvolving
public interface DynamoDbStreamsDeserializationSchema<T> extends Serializable, ResultTypeQueryable<T> {
    ...
    /**
     * Deserializes a DynamoDb Streams record. If the record cannot be deserialized, {@code null} may be
     * returned. This informs the Flink DynamoDb Streams Consumer to process the DynamoDb Streams record without
    ...
    static <T> DynamoDbStreamsDeserializationSchema<T> of(DeserializationSchema<T> deserializationSchema) {
        return new DynamoDbStreamsDeserializationSchemaWrapper<>(deserializationSchema);
    }
}
